...
export default function AudioPlayer() {
    ...
    function skip(delta) {
        ...
        if (newTime < 0) el.currentTime = 0;
        else if (el.duration && newTime > el.duration) el.currentTime = el.duration;
        ...
        <div style={{ maxWidth: 720, margin: "24px auto", fontFamily: "sans-serif" }}>
            ...
            <div style={{ display: "flex", gap: 16 }}>
                <div style={{ flex: 2 }}>
                    ...
                    <audio ref={audioRef} src={src} preload="metadata" style={{ width: "100%" }} controls={false} />

                    <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 12 }}>
                        <button onClick={() => skip(-10)}>« 10s</button>
                        <button onClick={togglePlayPause}>{isPlaying ? "Pause" : "Play"}</button>
                        <button onClick={() => skip(10)}>10s »</button>
                        <div style={{ marginLeft: "auto" }}>{fmtTime(currentTime)} / {fmtTime(duration)}</div>
                    </div>

                    <input type="range" min={0} max={duration ? duration : 0} step={0.01} value={currentTime} onChange={onSeek} style={{ width: "100%", marginTop: 10 }} />
                </div>
                    ...
                    <ul style={{ listStyle: "none", padding: 0, margin: 0, maxHeight: 300, overflowY: "auto" }}>
                        {files.map(f => (
                            <li key={f.key} style={{ marginBottom: 6 }}>
                                <button onClick={() => onSelectTrack(f.key)} style={{ background: "none", border: "none", padding: 0, textAlign: "left", cursor: "pointer", color: currentKey === f.key ? "blue" : "black" }}>
                                    {f.name}
                                </button>
                            </li>
                        ))}
